...
function Contact() {
    ...
    async function handleSubmit(event) {
        ...
        if (!userInput.name || !userInput.email || !userInput.message) {
            toast.error("All input feild required")
        }
                ...
                <form onSubmit={handleSubmit} className='rounded-lg border-2 border-yellow-400 lg:w-[450px] w-[90%] md:w-1/2 h-fit p-7 flex flex-col gap-5  bg-white text-black shadow-lg'>
                    ...
                    <div className='flex flex-col  w-full gap-4 '>
                        <div className='flex flex-col  w-full gap-2 '>
                            ...
                            <input type="text" placeholder="Enter Your Name" name='name' id='name' className="input input-bordered input-primary w-full text-white" onChange={handleUserInput} />
                        </div>
                        <div className='flex flex-col  w-full gap-2' >
                            <label htmlFor="email" className='font-semibold text-xl text-slate-500'>Email</label>
                            <input type="email" placeholder="Enter Your Email" name='email' id='email' className="input input-bordered input-primary w-full text-white " onChange={handleUserInput} />
                        </div>
                        <div className='flex  flex-col  w-full gap-2'>
                            <label htmlFor="message" className='font-semibold text-xl text-slate-500'>Message</label>
                            <textarea name='message' id='message' className="textarea textarea-primary  text-white" placeholder="Your Query" onChange={handleUserInput}></textarea>
                        </div>
                    </div>
